...
import History.ListingCreator;
import History.ListingKeeper;
...
public class Main
{
    public static void main(String[] args)
    {
        ...
        Listing initialListing = new Listing("1", null, null, "owner1", "2024-12-31", false, "Initial Description", false);
        ListingCreator creator = new ListingCreator(initialListing.getToken());
        ListingKeeper keeper = new ListingKeeper();

        keeper.saveState(creator.saveToToken());

        Listing updatedListing = new Listing("1", null, null, "owner1", "2025-01-15", true, "Updated Description", false);
        creator.setListing(updatedListing.getToken());

        keeper.saveState(creator.saveToToken());

        creator.restoreFromToken(keeper.restoreState());
        System.out.println("Restored Listing Description: " + creator.getListing().getDescription());
